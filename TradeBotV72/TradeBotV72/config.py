"""
config.py — v7.3 updated defaults

CHANGES from v6.1:
  MAX_DAILY_LOSS_PCT  1.5 → 2.0  (gives bot room to recover from early losses)
  GOLD_RR_RATIO       1.5 → 2.0  (base RR raised for better P&L per trade)
  GOLD_RISK_PCT       1.0 → 1.0  (unchanged — dynamic sizing handles scaling)
"""
import os
...
CONFIG = {
    ...
    "gold_rr_ratio":        parse_float(os.getenv("GOLD_RR_RATIO"),        2.0),   # v7.3: 1.5 → 2.0
    ...
    # ── Global Risk ───────────────────────────────────────────────────────────
    "gold_scalp_rr":            parse_float(os.getenv("GOLD_SCALP_RR"), 2.0),   # v7.3: 1.5 → 2.0
    "max_open_trades":          2,
    "max_trades_per_direction": 1,
    "max_daily_loss_pct":       parse_float(os.getenv("MAX_DAILY_LOSS_PCT"), 2.0),   # v7.3: 1.5 → 2.0
}
